...
* with a few minor modifications (search for 'AA' to find them) from A. Genz
* and some other ones from A. Bienvenue (search for 'AAB')
*
* Author:
*          Alan Genz
...
*-----------------------------------------------------------------------
      ...
      SUBROUTINE SADMVNG( N, IPUISS, PUISS,
     &                    LOWER, UPPER, INFIN, CORREL, MAXPTS,
...
*          Pullman, WA 99164-3113
*          Email : alangenz@wsu.edu
*     [AAB] This subroutine has been adapted to compute
*          int( f(x)*x_i^k ) over prod_i([LOWER_i,UPPER_i])
*     where f is a multivariate gaussian low density, instead of
*          int( f(x) ) over prod_i([LOWER_i,UPPER_i])
*
*  Parameters
*
*     N      INTEGER, the number of variables.
*     IPUISS INTEGER, the index of the coordinate that is added to the
*            integrand [AAB]
*     PUISS  INTEGER, the power of x_IPUISS in the integrand [AAB]
...
*
     ...
     &     OLDVAL, D, E, MVNNIT, MVNFNC
      ...
      IF ( N .GT. 20 .OR. N .LT. 1 ) THEN
         ...
         RETURN
      ENDIF
      INFORM = MVNNIT( N, IPUISS, PUISS,
     &                 CORREL, LOWER, UPPER, INFIN, INFIS, D, E )
      M = N - INFIS
      IF ( M .EQ. 0 ) THEN
         VALUE = 1
         ERROR = 0 
...
*
      INTEGER N, IPOWER, POWER, IPUISS, PUISS, INFIN(*), INFIS
      ...
      SAVE D1, E1, A, B, INFI, COV, IPOWER, POWER
      DI = D1
      EI = E1
      PROD = E1 - D1
      IJ = 1
      ...
      END DO
*     [AAB]
      PROD = PROD*(Y(IPOWER)**POWER)
      MVNFNC = PROD
      RETURN
*
*     Entry point for intialization.
*
      ENTRY MVNNIT(N, IPUISS, PUISS,
     &             CORREL, LOWER, UPPER, INFIN, INFIS, D, E)
      MVNNIT = 0
      POWER = PUISS
      IPOWER = IPUISS
...
*
      CALL NCVSRT(N, LOWER,UPPER,CORREL,INFIN,Y,
     &            INFIS,A,B,INFI,COV,D,E,IPOWER)
*      PRINT *,'INIT.IPOWER=',IPOWER
      D1 = D
      E1 = E
      END
            ...
            IF ( INFI(I) .GE. 0 ) THEN 
               DO J = 1,I-1
                  IF ( INFI(J) .LT. 0 .and. iflag .eq. 0) THEN
                     CALL RCSWAP(J, I, A, B, INFI, N, COV, IPOWER)
                     iflag = 1
                  ENDIF
               END DO
            ENDIF
...
* on garde toujours la coordonnee IPOWER a l'exterieur, en faisant
* semblant d'obtenir le min pour cette coordonnee.
*
*  MINISPOWER vaut 1 si le JMIN est IPOWER
                  IF ( MINISPOWER .EQ. 0 ) THEN
                     IF ( EMIN - DMIN .GE. E - D 
     &                    .OR. J .EQ. IPOWER ) THEN
                        IF ( J .EQ. IPOWER ) MINISPOWER = 1
                        JMIN = J
            ...
            END DO
            IF ( JMIN .NE. I) THEN
               CALL RCSWAP(I, JMIN, A, B, INFI, N, COV, IPOWER)
            ENDIF
*
...
*
      END
*--------------------------------------------------------------------------
      SUBROUTINE RCSWAP(P, Q, A, B, INFIN, N, C, IPOWER)
*
